...
class GeneSigDistance {
    ...
    void CalculateIDF()
    {
        cout << "Calculating kmer IDF" << endl;
        for (size_t i = 0; i < kmers.size(); i++) {
            int count = 0;
            for (size_t j = 0; j < geneSequences.size(); j++) {
                if(geneSequences[j].find(kmers[i]) != std::string::npos) {
                    count++;
                }
            }
            // cout << kmers[i] << endl;
            kmerIDF[kmers[i]] = log10((float)geneNum / (float)count);
        }
        // cout << "AAAAA IDF:" << kmerIDF["AAAAA"] << endl;
    }

    void InitGenes(string genePath)
    {
        cout << "Reading all genes..." << endl;
        GetGenes(genePath);
        CalculateIDF();
    }
	...
	void CalculateGeneSigsThread(int threadIndex, int threadNum) {
		...
		for (int i = startIndex; i <= endIndex; i++) {
            ...
            map<string,int> geneKmersMap = UT_GenomeKit::GetKmersMap(geneSequences[i], kmerLen);
			vector<float> geneSig(kmerSigLen, 0.0);

            for (map<string,int>::iterator it = geneKmersMap.begin(); it != geneKmersMap.end(); ++it) {
                float tf = (float)it->second / (float)geneKmersMap.size();
                float idf = kmerIDF[it->first];
                float tfidf = tf * idf * 1000;
                vector<float> kmerSig = kmerSigDict[it->first];
            	for (int j = 0; j < kmerSigLen; j++) {
            		geneSig[j] += kmerSig[j] * tfidf;
            	}
            }
	...
	void GenerateGeneSigs(int threadNum) {
...
int main(int arg, char *argvs[]) {
	...
	gsd.CalculateGenePairDistScore(threadNum, 3000);
	gsd.SaveTrecEval(geneSigPath + "_tfidf_trec_result");
