   ...
   public:
    ...
    void TrainEach(int *inputWordsIndex, int inputSize, int outputWordIndex, int threadIndex) {
        ...
        while (1) {
            ...
            for (int k = 0; k < inputSize; k++) {
                for (int i = 0; i < hlsize; i++) {
                    WIH[inputWordsIndex[k]][i] += (WIHe[threadIndex][i]) + beta * (WIHe_pre[threadIndex][i]);
    ...
    void EvalLoss(int *inputWordsIndex, int inputSize, int outputWordIndex, int threadIndex) {
        int iterCount = 0;
        memset(WIHe_pre[threadIndex], 0, sizeof(WIHe_pre[0][0]) * hlsize);
        int outputLayerSize = vocabVec[outputWordIndex]->codeArray.size();
        float error = 0;
        ...
        // cout << "Eval error: " << error << endl;
        globalError += error;
    }

    void EvalLoss(int sentenceIndex, int threadIndex) {
        for (int i = 0; i < wordIndexInSentence[sentenceIndex].size(); i++) {
            int outputIndex = wordIndexInSentence[sentenceIndex][i];
            int inputArray[20];
            int inputCount = 0;
            ...
            if (inputCount == 0) continue;
            EvalLoss(inputArray, inputCount, outputIndex, threadIndex);
            // trainingWordCount++;
        }
    }
