    ...
    void TrainingSentence(int sentenceIndex, int threadIndex)
    {
        for (int i = 0; i < wordIndexInSentence[sentenceIndex].size(); i++) {
            ...
            for (int j = 1; ; j++) {
                ...
                if (inputCount == shiftSize * 2 ||
                    !(i - j > 0 && i + j < wordIndexInSentence[sentenceIndex].size())) {
                    break;
                }
            }
    ...
    void TrainingThread(int threadIndex)
    {
        ...
        for (int i = 0; i < maxIteration; i++) {
            for (int i = beginIndex; i < endIndex; i++) {
                ...
                printf("\rProgress: %.3f%%",
                       (float)trainingSentenceCount / (float)wordIndexInSentence.size() / maxIteration * 100.0);
                fflush(stdout);
            }
        }
    }
